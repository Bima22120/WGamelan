...
class GamelanSampler:
    ...
    def _generate_mallet_transient(
        ...
        transient_ms = 35.0 if is_gong else (8.0 if is_kettle else (12.0 if hardness > 0.65 else 20.0))
        ...
        decay_rate = 140.0 if is_kettle else (100.0 if hardness > 0.65 else (50.0 if not is_gong else 30.0))
    ...
    def synthesize_note(
        ...
        if is_gong:
            ...
            # Comprehensive bronze kettle partials: fundamental + dome flexural modes + rim modes
            # Crucial: covers 1 kHz to 8 kHz to restore missing upper spectrum
            partials = [1.0, 1.480, 2.160, 2.880, 3.750, 4.850, 6.240, 8.120, 10.450, 12.800]
            decay_rates = [1.0, 1.4, 2.0, 2.8, 3.8, 4.8, 6.0, 7.5, 9.0, 11.0]
            ...
            total_dur = max(duration_sec, 1.5)
            hardness = max(self.instrument.strike_hardness, 0.75)
            ombak_rate = 3.6
        else:
        ...
        transient = self._generate_mallet_transient(total_dur, hardness=hardness, is_gong=is_gong, is_kettle=is_kettle)
        signal += transient * (0.35 * vel_norm)

        # Acoustic Bamboo Chamber Resonator (Bumbung)
        # Applied ONLY to metallophone bars (Saron/Demung); bypassed for Bonang & Gong
        if not is_gong and not is_kettle:
